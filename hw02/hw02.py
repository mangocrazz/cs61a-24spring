...
def accumulate(fuse, start, n, term):
    ...
    if n == 0:
        return start
    def start_accumulator(fuse,n,term):
        if n == 1:
            return term(1)
        return fuse(term(n),start_accumulator(fuse,n-1,term))
    return fuse(start,start_accumulator(fuse,n,term))
...
def interleaved_sum(n, odd_func, even_func):
    ...
    def help_func(k):
        if k > n:
            ...
            return odd_func(k) + even_func(k+1) + help_func(k+2)
...
def count_coins(total):
    ...
    def helper(total,coin):
        ...
        if total < 0 or coin is None:
            ...
            return helper(total-coin,coin) + helper(total,smaller(coin))
